...
**Contact Center ****Lead Scoring *****(Erick A.)***
...
MIAP
Met with Eniram to discuss a POC for taking over Eniram IoT servers with MIAP IoT servers (CAR planned for next year).
Met with GMO and IT teams to discuss replacing the Meyer Turku Energy Management App (MEMS) with MIAP as a real-time onboard application with streaming data enrichment and ML. All parties agreed to move forward. Next steps include requesting funding from the IT Newbuild team and targeting Icon of the Seas for 2027.
Met with the deployment team and reviewed call-time optimization modeling.
...
**Finished Integral testing and bug fixing of code improvement initiative. successfully running in QA, will keep a couple more weeks running there before replacing production code. Still pending to update the dashboard to use latest changes.****
Tested changes include:****
-  ****databricks**** staging environments support [dev, ****qa****, ****prd****]****
****- ****dbr**** 13.3 -> 17.3****
- ****pyspark**** 3.4 -> 4.0  ****
- ****mlflow**** 2.5 -> 3.0, ****
- ****mlflow**** model registry -> ****mlflow**** ****databrick****-uc registry****
- pandas fast parquet -> to arrow****
- replace savings location from  dffs://filestore  to ****adls**** volume****
- bugs Fixing  ****
- improve code logic****
- code refactor****
- add cross-validation option to model training and feature selection****
- unit tests**
...
Doug B., Kevin D., Eswar T., Mert E., Ben F., and Erick A.
...
E-Commerce Customer Targeting: SSC Marketing
...
Contact Center: **Workforce Planning – International and Casino – App Updates**
...
**Contact Center: ****Workforce Planning – North America (Celebrity).**
...
Revenue Management: RCI - SAILING ENVIRONMENT
...
RCI Revenue Management: All efforts this week have been made to Inventory and Pricing (PRE) automation processes to ensure that SharePoint authentication continues to work after Microsoft disables existing access. Replaced authentication with MSAL/Graph API and tested completely.
...
RCI Revenue mgmt.: **RCI / CEL / SSC \ Track Optimization**
...
RCI revenue Mgt:  **Category****-Gapping 3.0**
...
PCP Pricing Automation: **Add shipboard WTS bin for updated +5% STLY Revenue optimization targets + test in lower env for output analysis**
• Shipboard promos were added into the optimization routine for the updated +5% STLY Revenue targets. The shipboard WTS bin had already been added into the model training notebooks in the process of adding in the RBC bundles in the past. Now, the optimization routine for the +5% STLY targets had to be updated to also include this bin. After this change, the optimization was run in order to get some example outputs of the price recommendations to then be analyzed and looked through with the business team.sss
...
PCP Pricing Automation: Mas promotions writeback.
...
PCP Pricing Automation: CRF Automation & RBC Testing
...
Kevin D., Lamis A., Eddie B.

Revenue Management (CEL & RCI): Track Discussion
Across both the Tri-Branded RMA and CEL Track discussions, there was strong strategic alignment that **getting booking tracks right is foundational to revenue performance**, because tracks sit at the intersection of demand forecasting, pricing, capacity management, and long-term value preservation. In both forums, teams emphasized the importance of anchoring decisions to an **SPI****-informed view of high****-performing booking behavior**, while recognizing that SPI is not a static prescription but a contextual signal that must be applied dynamically based on current booking position, capacity consumed (especially by groups), and proximity to sail date. A common theme was that **tracks and demand forecasts must ultimately converge**—persistent divergence signals a modeling or assumption issue rather than a business choice—and that corrective actions should be **proportional, momentum****-preserving, and coordinated with pricing** to avoid value-destructive rate moves. Both groups highlighted that early-window guidance is more reliable than closer-in signals, that aggregation and granularity matter materially to interpretation, and that sailings require **tailored pacing based on elasticity, capacity evolution, and business environment**, not one-size-fits-all curves. Collectively, the conversations reinforced that disciplined, dynamic track management is strategically critical: it governs how aggressively the business sells, protects long-term yield, and ensures pricing, forecasting, and optimization operate as a coherent system rather than competing levers.
...
David W., Erick A., and Ben F.

HR: CAM Demand forecasting
...
The project will be executed shoreside in **Port of Miami / Miramar** through a joint effort between the CAM Planning team and RCG’s centralized AI team. This sCAR specifically requests to hire a **Data Science contractor (bill rate: $69/hour)** for **36 weeks (through year-end)** to accelerate delivery and support the iterative build, testing, and tuning required for a first-of-its-kind planning capability. Importantly, the effort is intentionally **exploratory and constrained in scope** to help the CAM Planning and AI teams better understand the required **infrastructure, data pipelines, and AI/modeling needs** for a scalable long-term solution; if scope expands based on findings and adoption, a **full CAR may be submitted next year**. Success will be measured through: (a) **forecast refresh frequency** (daily automated updates versus semi-monthly manual cycles), (b) **planning cycle-time reduction** (hours required per cycle), (c) **forecast accuracy and stability** via back-testing/validation and variance tracking, (d) **earlier risk identification** through improved visibility into projected staffing gaps, and (e) **downstream efficiency gains** evidenced by fewer avoidable over-hires, reduced rescheduling, less onboarding rework, and better-timed travel bookings.
...
**Ambitious deliverable list for one contractor.** Six deliverables are listed (demand model, 18-month forecast, dashboards, variance reporting, KPI automation, forecast validation) — all for one person over 36 weeks with unknown data pipeline readiness. What is the prioritized order, and what is the MVP if only 2–3 are achievable?
...
**Relationship ****to**** Crew Air workstream.** The broader Crew Travel project includes a separate Crew Air optimization initiative (also Angela-sponsored). The sCAR lists "better-timed travel bookings" as a downstream benefit. Is there an actual integration plan between these two efforts, or is that aspirational?
...
RCI Revenue management: SPI Factor Model (Track Shape)
