...
Supply Chain (IBP)
Key accomplishments
• Automated and productionized reporting for Health Expiration File Generation, ingesting Excel and outputting formatted .txt files for Crunchtime, eliminating manual steps and saving Paige ~1 hour per month.
• SSC Actual Quantity Needed: Revised logic to use safety stock instead of min par proposed.
• SSC Min and Max Par: Added safety stock to the table and recalculated max par proposed by category (e.g., Food ×2, Uniforms ×1).
• RCI Min and Max Par: Added order frequency type to the calculation of average days until next ship load date so products with different frequencies are handled appropriately.
• Added warehouse transfer orders into SSC order creation so they are captured in the Actual Quantity Needed formula.
• Investigated Fanny’s record count request for missing SSC items and clarified that prior checks used the spend report, while our standard is the consumption report (instances/occurrences since 2021).
• Verified that a product with previously high RCI/CCI forecasts is now aligned with historical consumption following Ben’s model enhancements.
• Together with Ben, investigated a question from supply chain team about “incorrect” demand forecasts relative to historical consumption. We traced the difference to a conversion factor in a business supplied excel file that is used to convert demand forecasts to a different amount when specified by the procurement team. We provided the Supply Chain team with the results of our investigation and followed up with our contact to confirm whether those factors are now stale.
• Corrected CocoCay logic regarding ports and their products; tested HF&B with CocoCay SharePoint files and added the UOM items master into the notebook to avoid null UOM values.
• Began work on reducing duplicate ports by resolving to region instead of port.
In progress / next
• Using backtested results from the new RCI/CCI HF&B demand model to calculate what the Finance tool would have produced for November had the new model been in place; results will be shared with Juan Lopez next week.
• Identifying all upstream and downstream tables for all pipelines.
• Investigating why RCI/CCI order creation ship load dates stop at January 2026 while MLS shows dates through January 2027, so Brenda has sufficient future coverage for all ships and products.
• Recalculating backtested dates based on the free-of-charge items logic (pending Medical validation).
• Investigating Fanny’s finding that Legend does not appear in container load date files.
• Creating volatility reports for RCI/CCI Min/Max Par.
• Continuing work on the SSC Finance Tool.
...
**RCI Revenue Management:**** Data Driven GTY Replenishment Optimization**
...
**CEL Revenue Management: ****AD HOC | CEL GTY-LEAD 2.0**** and DART**
...
**RCI**** Revenue Management:**** Price Optimization**
...
**CEL**** Revenue Management**** | Productionizing Upgraded CEL Elasticity**
...
**RCI**** Revenue Management**** | PRE | Dynamic PRE ****CapsTESTING**** in QA**
...
**RCI Revenue Management ****| Analyze Promotion Code Impact on Sailings & Integrate Rules into PRE**
