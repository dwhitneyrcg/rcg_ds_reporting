...
RCI Revenue Management: **Promotion Optimization (Track****-based rules)**
...
**SSC Revenue Management: ****Universal A/B Framework**
...
CEL Revenue Management | **CEL Elasticity Model with DART:**
...
PCP Pricing Automation - **Work with Cihan to debug and improve optimization outputs for Waterpark PRE**
- Cihan’s optimization was having errors that needed to be debugged. We met and reviewed the code; I added additional code as guardrails to cover edge cases that can cause some errors. Further debugging was needed to handle issues of the recommended price always reaching the max price constraint. We believed this could be due to 2 possible reasons:
...
PCP Pricing Automation - **Automate CRF****
**- The automation of the CRF table for marketing is created. A meeting was had with marketing team to clarify the required columns for this CRF table. After joining a set of different tables together, the only column still missing is a column for RBC flag. This is being created on my end to be joined with the rest of the table as a way of flagging RBC sailings. Once this is prepared, a couple other things are also needed to complete the CRF automation:
...
PCP Pricing Automation - Update OBR FS & optimization with fixed source tables from DE
The following tables (which were previously view tables, therefore having long runtimes when being used for EDA, feature store updates, and optimization runs) were updated and optimized by the DE to no longer be view tables and function more efficiently in production:
...
MIAP
This week
Implemented logging changes in the REST API and merged them into master.
Pushed autoloader changes from the DE workspace to production.
Pushed streaming changes to production for the shipboard table.
Replaced the DLT pipeline with structured streaming for shipboard.
Authorized credentials for Zerobus, but the stream is still not working, likely due to security restrictions.
Next week:
...
Extending the **CLV table from 2024 to include 2025**:

**Ticket Revenue:**

Waiting on **Revenue Planning** to refresh the **vcap**** source** to obtain **validated Ticket data** for 2025.
...
**Project Axiom****: Generative AI SEAT Medallia: Key ****Project Updates**
...
**Project Axiom****: Generative AI SEAT Medallia:**
...
Medallia Genie: The team is creating an evaluation dataset to baseline Genies ability to respond to questions. If Genie is found to be capable of answering a broad spectrum of questions, it may be integrated to the Axiom web app.
...
RCI Revenue Management: Refactoring
