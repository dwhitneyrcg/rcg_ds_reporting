...
└── prompts/                              # AI prompt definitions (→ skills)
...
Prompts are stored as `.docx` files in `knowledge/prompts/`. This creates friction:
...
### Phase 1: Foundation (Current → Stabilize)
...
- [ ] Convert prompts from `.docx` to `.prompt.md`
- [ ] Extract mapping dictionaries from Python code to master `.md` files

### Phase 2: Step 2 Pipeline
- [ ] Design entity edit strategy (what fields, what format, what rules)
- [ ] Build Step 2 prompt(s) for entity propagation
- [ ] Build changelog generator
- [ ] Implement automated validation checks (YAML, wikilinks, orphan refs)
...
1. **Input format migration:** Can team members submit weekly updates as markdown or a structured form instead of `.docx`? This would eliminate parsing complexity. Response: Yes, the inputs can be Markdown. 
2. **Two-PR vs. one-PR per cycle:** Should Step 1 output be reviewed and merged before Step 2 runs, or should the full cycle be a single PR? Two PRs add process overhead but prevent cascading errors. Response: Yes, the first PR will be vetted by the Data Science leaders for the normal weekly update to Leaders, but updating the knowledge base should be reviewed by the team, likely in multiple smaller PRs.
3. **Granularity of entity updates:** How much history should entity files retain? Append-only tracking tables will grow indefinitely — define a rolling window or archival strategy. Response: Good question, for now everything. We may 
